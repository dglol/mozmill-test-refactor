...
function getBrowserWindow(aWindowClass, aOpenIfNone) {
  ...
  aWindowClass = aWindowClass || require('../ui/browser').BrowserWindow;
  ...
  if (win)
    browser = new aWindowClass(win);
  else
    browser = openBrowserWindow(aWindowClass);
...
function openBrowserWindow(aWindowClass) {
  ...
  return new aWindowClass(hWindow.OpenBrowserWindow());
}
 ...
 * @param {Object} [aFilterValue] Value to pass to the filter function.
 ...
 */
function _getWindows(aEnumerator, aFilterCallback, aFilterValue, aStrict) {
  // Set default
  ...
  while (aEnumerator.hasMoreElements()) {
    ...
    if (!aFilterCallback || aFilterCallback(window, aFilterValue)) {
      windows.push(window);
    }
  }
...
function getWindowsByAge(aFilterCallback, aFilterValue, aStrict) {
  var windows = _getWindows(services.wm.getEnumerator(""),
                            aFilterCallback, aFilterValue, aStrict);
 ...
 */
function getNewestWindow(aFilterCallback, aFilterValue, aStrict) {
  var windows = getWindowsByAge(aFilterCallback, aFilterValue, aStrict);
 ...
 * @returns {DOMWindow} The window, or null if none found and aStrict == false
 */
function getTopmostWindow(aFilterCallback, aFilterValue, aStrict) {
  var windows = getWindowsByZOrder(aFilterCallback, aFilterValue, aStrict);
  return windows.length ? windows[0] : null;
}
