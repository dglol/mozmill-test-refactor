...
function ChromeWindowWrapper(aDOMWindow) {
  ...
  if (typeof aDOMWindow === "undefined")
    return;
...
function ContentWindowWrapper(aContentWindow) {
  // If aContentWindow is undefined, this is a prototype and we should exit now.
  if (typeof aContentWindow === "undefined")
    return;
...
/**
 * Close all windows but leave the specified browser window open
 *
 * @memberOf windows
 * @param {BrowserWindow} aBrowserWindow
 *        Browser window which should not be closed. For Mac it can be 'null', if
 *        all windows should be closed.
 */
function closeWindows(aBrowserWindow) {
  // On non-Mac platforms there is no hidden window so we need a browser window
  if (!mozmill.isMac && (typeof aBrowserWindow === 'undefined'))
    throw errors.InvalidParameterError("A browser window has not been specified");

  driver.getWindowsByAge().forEach(function (win) {
    if (aBrowserWindow && aBrowserWindow.innerWindow === win)
      return;
 ...
 *        Native window instance to check
 * @returns {Boolean} True, if the window content has been loaded
 */
function isWindowLoaded(aWindow) {
  if (!aWindow)
    throw new errors.UnexpectedError("Window reference is not a valid window.");
...
// Export of functions
windows.closeWindows = closeWindows;
