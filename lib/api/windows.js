...
/**
 * Reset all windows and leave the specified browser window open
 *
 * @memberOf windows
 * @param {BrowserWindow} aBrowserWindow
 *        Browser window which should not be closed
 */
function resetWindows(aBrowserWindow) {
  if (typeof aBrowserWindow === 'undefined')
    throw errors.InvalidParameterError("A browser window has not been specified");

  driver.getWindowsByZOrder().forEach(function (win) {
    if (win !== aBrowserWindow.innerWindow)
      win.close();
  });
}
...
// Export of functions
windows.resetWindows = resetWindows;
