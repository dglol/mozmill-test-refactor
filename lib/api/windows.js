...
/**
 * Closes all windows except the ones specified.
 *
 * @param {Window[]} aExceptList
 *        List of windows which should not be closed
 */
function closeWindows(aWindowList) {
  driver.getWindowsByZOrder().forEach(function (win) {
    if (aWindowList.indexOf(win) !== -1)
      win.close();
  });
}
 ...
 */
function isWindowLoaded(aNativeWindow) {
  return ("mozmillDocumentLoaded" in aNativeWindow) &&
         aNativeWindow.mozmillDocumentLoaded;
}
