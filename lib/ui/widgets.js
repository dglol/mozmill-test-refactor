...
function Element(aLocatorType, aLocator, aOwner) {
  ...
  if (aOwner) {
    if (aOwner instanceof Element) {
      ...
      this._controllerObject = aOwner._controllerObject;
    }
    else {
      ...
      // Content documents don't have a unique controller, only the chrome window they're
      // displayed in. So if this is a content element, operations on it will always
      // route through the topmost browser window. This implies content must be currently
      // displayed for the Elements to be valid.

      // If the document is Chrome...
      if (this._document.contentType.match("application/.*"))
        // ...it gets its own controller
        this._controllerObject = new mozmill.controller.MozMillController(this._document.defaultView);
      else
        // ...otherwise, it's content. We (and any of our "owned" elements) will
        // always use the controller of the topmost browser window.
        this._controllerObject = undefined;
    }
  }
  else {
    // Not supplied at all, so we're top level and our doc is the current window.
    this._owner = undefined;
    this._controllerObject = mozmill.getBrowserController();
 ...
 * @fieldOf widgets.Element#
 */
Element.prototype.__defineGetter__("isValid", function Element_getIsValid() {
