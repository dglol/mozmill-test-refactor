...
/**
 * RadioGroup represents the visual control of a radiogroup node in a HTML or
 * Chrome document.
 *
 * @class An Element that corresponds to a radiogroup node in an HTML or Chrome document
 * @constructs
 * @extends widgets.Widget
 * 
 * @param {String} aLocatorType
 ...
 *   a document. Other members of the map are owned by their parent Elements.
 */
function RadioGroup(aLocatorType, aLocator, aOwner) {
  Widget.apply(this, arguments);

  this.updateRadioButtons();
}
inheritance.extend(RadioGroup, Widget);


/**
 * Update the internal list of available radio buttons
 *
 * @memberOf RadioGroup
 * @param {String} [aAttributeName=id]
 *                 Attribute name of the buttons to use to hash the key
 */
RadioGroup.prototype.updateRadioButtons =
function RadioGroup_updateRadioButtons(aAttributeName) {
  var self = this;

  if (typeof aAttributeName === "undefined")
    aAttributeName = "id";

  this._buttons = new Array();
  this._length = 0;

  // Iterate over all radio button children
  this.node._radioChildren.forEach(function (button) {
    self._buttons[button[aAttributeName]] = new Button("node", button, self);
    self._length++;
  });
}
...
/**
 * Return a hash of available buttons
 *
 * @memberOf RadioGroup
 * @returns {Element[]} Hash of radio buttons
 */
RadioGroup.prototype.__defineGetter__("buttons", function RadioGroup__getButtons() {
  return this._buttons;
})


/**
 * Return the identifiers of available radio buttons
 *
 * @memberOf RadioGroup
 * @returns {String[]} Identifiers of radio buttons in the radiogroup
 */
RadioGroup.prototype.__defineGetter__("keys", function RadioGroup__getLength() {
  var keys = new Array();
