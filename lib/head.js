...
/**
 * Saved-off value of the current module as specified in setup()
 *
 * @private
 */
var testModule = null;
...
function setup(aModule, aFlags) {
  ...
  // Store the current test module for usage outside of setup/teardown
  testModule = aModule;
  ...
  /**
   * Helper to be able to directly create a content window.
   *
   * @name ContentWindow
   * @type windows.ContentWindowWrapper
   * @memberOf module
   */
  aModule.ContentWindow = windows.ContentWindowWrapper;
   ...
   */
  aModule.browser = browser.getBrowserWindow();

  // Reset the browser into a known state
