...
const { UnknownEntityError, UnknownPropertyError } = require('api/errors');
...
function getEntity(aDTDs, aEntityId) {
  ...
  if (!node) {
    ...
    throw new UnknownEntityError(message, filename, frame.lineNumber, frame.name);
  }
...
function getProperty(aURL, aProperty) {
  ...
  try {
    ...
    throw new UnknownPropertyError(message, filename, frame.lineNumber, frame.name);
  }
}
