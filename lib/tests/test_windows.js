...
const {BrowserWindow} = require("../mixins/browser");
...
function testWindows() {
  ...
  var browser2 = new BrowserWindow(browser.findWindow(driver.filterWindowByType,
                                                      "navigator:browser"));
  ...
  browser2.close();
  expect.equal(driver.getWindowsByZOrder().length, 1, "A single window is visible.");
  ...
  expect.equal(persisted.windowHandled, true, "Callback has been correctly processed.");
  expect.equal(driver.getWindowsByZOrder().length, 2, "Two windows are visible.");
  win.close();
}
